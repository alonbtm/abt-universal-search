export class FallbackManager {
        ...
        if (this.config.enableCachedResults) {
            this.registerStrategy({
                ...
                executor: {
                    ...
                    canExecute: (error, context) => this.hasCachedData(this.getCacheKey(context.metadata?.query || '', context)),
    ...
    async executeStrategy(error, query, context) {
    ...
    async executeStrategyWithTimeout(strategy, query, context) {
    ...
    async executeCachedResults(query, context) {
        ...
        return {
            ...
            suggestions: [
                ...
                'Try refreshing when network is available'
            ]
        };
    }
    async executeSimplifiedMode(query, context) {
        ...
        for (const [, cached] of Array.from(this.cache)) {
            const matches = cached.data.filter((item) => this.simpleStringMatch(item, query));
    ...
    async executeOfflineMode(query, context) {
        if (!this.isOffline) {
        ...
        let cacheCount = 0;
        for (const [, cached] of Array.from(this.cache)) {
            const matches = cached.data.filter((item) => this.advancedOfflineMatch(item, query));
            if (matches.length > 0) {
    ...
    async executeEmptyResults(query, context) {
        return {
            success: false,
            data: [],
    ...
    generateEmptyResultsSuggestions(context) {
        const suggestions = [
    ...
    simpleStringMatch(item, query) {
        const searchText = query.toLowerCase();
        const itemText = JSON.stringify(item).toLowerCase();
        return itemText.includes(searchText);
    }
    advancedOfflineMatch(item, query) {
    ...
    normalizeError(error, strategyName) {
        return {
    ...
    updateSuccessMetrics(fallbackTime) {
    ...
    getAvailableStrategies(error, context) {
    ...
    cacheResults(query, context, data, reliability = 1.0) {
        if (!this.config.enableCachedResults)
            return;
        const cacheKey = this.getCacheKey(query, context);
    ...
    testStrategy(strategyName, query, context) {
