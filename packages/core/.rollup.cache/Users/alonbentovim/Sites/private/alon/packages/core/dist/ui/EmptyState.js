...
export class EmptyState {
     ...
     */
    async show(message, suggestions) {
        if (!this.isInitialized) {
            await this.init();
        }
     ...
     */
    setIllustration(content, type = 'text') {
        if (!this.illustrationElement)
            return;
    ...
    applyAccessibilityAttributes() {
        if (!this.emptyStateElement || !this.accessibilityManager)
            return;
    ...
    setupKeyboardNavigation() {
        if (!this.emptyStateElement || !this.keyboardHandler)
            return;
    ...
    announceToScreenReader(message) {
        if (!this.screenReaderManager || !this.config.announceChanges)
            return;
    ...
    updateContent(message, suggestions) {
        // Update message
    ...
    generateContextualMessage(query, options) {
    ...
    generateContextualSuggestions(query, options) {
        const suggestions = [];
        if (query.trim()) {
    ...
    showEmptyState() {
        if (!this.emptyStateElement || this.isVisible)
            return;
