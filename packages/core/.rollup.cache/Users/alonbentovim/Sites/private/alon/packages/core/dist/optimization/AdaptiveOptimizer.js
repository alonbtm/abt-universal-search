...
class PatternRecognizer {
    ...
    learnFromOutcome(patternId, action, improvement) {
        const pattern = this.patterns.get(patternId);
        if (!pattern)
            return;
    ...
    extractSignature(metrics) {
        ...
        for (const metric of metrics.slice(-5)) { // Last 5 measurements
            signature.push(responseTimeMax > 0 ? metric.responseTime.average / responseTimeMax : 0, memoryMax > 0 ? metric.memory.heapUsed / memoryMax : 0, throughputMax > 0 ? metric.throughput.requestsPerSecond / throughputMax : 0, metric.cache.hitRate, metric.errors.errorRate);
        }
        return signature;
    }
    ...
    generateRecommendationFromPattern(pattern) {
        ...
        const actionImprovements = new Map();
    ...
    generateHeuristicRecommendation(metrics) {
...
class BayesianOptimizer {
    ...
    addObservation(parameters, objective) {
    ...
    getBestParameters() {
        if (this.observations.length === 0)
            return null;
        return this.observations.reduce((best, obs) => obs.objective > best.objective ? obs : best).parameters;
    }
    randomSample() {
        const parameters = {};
    ...
    acquisitionFunction(parameters) {
    ...
    estimateUncertainty(parameters) {
    ...
    calculateVariance(parameters) {
        // Simplified variance calculation
        const similarities = this.observations.map(obs => this.calculateParameterSimilarity(parameters, obs.parameters));
...
export class AdaptiveOptimizer {
     ...
     */
    generateRecommendations(currentMetrics, historicalMetrics) {
     ...
     */
    learnFromResults(recommendation, beforeMetrics, afterMetrics) {
    ...
    detectAnomalies(metrics) {
        ...
        const responseTimeZScore = this.calculateZScore(latest.responseTime.average, historical.map(m => m.responseTime.average));
        const memoryZScore = this.calculateZScore(latest.memory.heapUsed, historical.map(m => m.memory.heapUsed));
        const errorZScore = this.calculateZScore(latest.errors.errorRate, historical.map(m => m.errors.errorRate));
    ...
    generateHeuristicRecommendations(currentMetrics, historicalMetrics) {
        const recommendations = [];
    ...
    generateReinforcementRecommendations(currentMetrics) {
    ...
    generateBayesianRecommendations(currentMetrics) {
    ...
    rankRecommendations(recommendations, currentMetrics) {
        return recommendations
            .map(rec => ({
            ...rec,
    ...
    calculateRecommendationScore(recommendation, currentMetrics) {
        ...
        for (const [objective, weight] of Object.entries(this.config.weights)) {
            const impact = recommendation.impact[objective] || 0;
    ...
    calculateZScore(value, historical) {
    ...
    extractParameters(recommendation) {
    ...
    updateSuccessRate() {
        const successfulOptimizations = this.optimizationHistory.filter(h => h.improvement !== undefined && h.improvement > 0).length;
        this.state.successRate = this.optimizationHistory.length > 0
            ? successfulOptimizations / this.optimizationHistory.length
            : 0;
    }
    updateConfidence() {
        this.state.confidence = Math.min(0.95, 0.1 + (this.state.successRate * 0.5) + (this.state.learningProgress * 0.4));
    }
    startLearningLoop() {
 ...
 */
export function createAdaptiveOptimizer(config) {
...
//# sourceMappingURL=AdaptiveOptimizer.js.map