...
export class APIAdapter extends BaseDataSourceAdapter {
    ...
    async connect(config) {
        ...
        if (config.auth) {
            try {
                ...
                throw this.createError(`Authentication failed: ${error.message}`, 'security', 'AUTH_FAILED', error);
            }
        }
    ...
    async query(connection, query) {
        ...
        return this.executeWithMetrics(connection.id, async () => {
            ...
            if (apiConnection.rateLimiter && this.config.rateLimit) {
                const limiter = rateLimiterFactory.getLimiter(apiConnection.baseURL, this.config.rateLimit);
            ...
            // Transform request
            const requestConfig = requestTransformer.transformRequest(query, this.config, apiConnection.defaultHeaders);
            ...
            try {
                ...
                if (this.config.cors?.autoFallback && corsHandler.isCORSError(error)) {
                    apiResponse = await corsHandler.handleCORSError(requestConfig, this.config, error);
                }
                else {
                    throw this.createError(`API request failed: ${error.message}`, 'network', 'REQUEST_FAILED', error, { query: query.original });
                }
            }
            ...
            const validatedResponse = await responseValidator.validateResponse(this.createMockResponse(apiResponse), this.config.response);
            ...
            if (responseValidator.isErrorResponse(validatedResponse)) {
                ...
                throw this.createError(`API error: ${errorMessage}`, 'adapter', 'API_ERROR', undefined, { query: query.original });
            }
            // Transform to search results
            const results = responseValidator.transformToResults(validatedResponse.data, this.config.response);
     ...
     */
    async executeStandardRequest(requestConfig) {
     ...
     * Cache results
     */
    cacheResults(cacheKey, results, ttlMs) {
