...
class AccessibilityAuditor {
    ...
    async auditComponent(ComponentClass, componentName, config = {}) {
    ...
    async exerciseComponent(component, ComponentClass) {
    ...
    generateRecommendations(validationResult, colorIssues) {
    ...
    calculateComponentScore(validationResult, colorIssues) {
        const totalIssues = validationResult.violations.length + colorIssues.length;
        const criticalIssues = [...validationResult.violations, ...colorIssues].filter(v => v.severity === 'error').length;
        ...
        if (violationTypes.includes('keyboard'))
            score -= 10;
        if (violationTypes.includes('aria'))
            score -= 8;
        if (violationTypes.includes('focus'))
            score -= 8;
    ...
    async runFullAudit() {
        ...
        try {
            ...
            const failedCriteria = allViolations.map(v => v.wcagCriteria).filter(Boolean);
            const report = {
                timestamp,
    ...
    extractPassedCriteria(results) {
        ...
        const failedCriteria = new Set(results.flatMap(r => r.violations.map(v => v.wcagCriteria).filter(Boolean)));
