...
export class AdvancedActionHandler {
  private config: Required<ActionHandlerConfig>;
  private statistics: ActionHandlerStatistics = this.initializeStatistics();
  private activeActions = new Map<string, { startTime: number; result?: SearchResult }>();
  private processingQueue: Array<{
    id: string;
    result: SearchResult;
    options: ActionProcessingOptions;
    resolve: (value: ActionProcessingResult) => void;
  ...
  public async processAction(
    ...
    try {
      ...
      const finalResult: ActionProcessingResult = {
        success: executionResult.success,
      ...
      if (this.config.debug) {
        console.log(
          `[ActionHandler] Action ${actionId} completed in ${processingTime.toFixed(2)}ms`
        );
      }
   ...
   */
  public registerActionCallback(
    event: string,
    callback: (result: SearchResult, context: any) => unknown | Promise<unknown>,
    options: {
      priority?: number;
      once?: boolean;
      timeout?: number;
    ...
    const subscriptionId = eventManager.subscribe(
      ...
      async data => {
        if (data && typeof data === 'object' && 'result' in data && 'context' in data) {
          const { result, context } = data as { result: SearchResult; context: any };
    ...
    if (this.config.debug) {
      console.log(
        `[ActionHandler] Registered callback for event '${event}' with ID ${subscriptionId}`
      );
    }
   ...
   */
  public async processActionsParallel(
    actions: Array<{ result: SearchResult; options?: ActionProcessingOptions }>,
    globalOptions: ActionProcessingOptions = {}
  ): Promise<ActionProcessingResult[]> {
    const startTime = performance.now();
    ...
    const promises = actions.map(({ result, options = {} }) =>
      this.processAction(result, { ...globalOptions, ...options })
    );
   ...
   */
  public getStatistics(): ActionHandlerStatistics {
   ...
   */
  public async cleanup(): Promise<{ cleaned: number; errors: number }> {
   ...
   */
  private async createActionContext(
    result: SearchResult,
    options: ActionProcessingOptions
  ): Promise<{ success: boolean; context?: any; error?: Error }> {
    try {
      const context = contextPreserver.createContext(
        result,
        '', // query will be filled by the context preserver
        {
      ...
      const preservationResult = contextPreserver.preserveContext(
        context,
        {
          encrypt: false,
          compress: false,
   ...
   */
  private async performActionCleanup(actionId: string, context: any): Promise<boolean> {
    try {
      const cleanupId = cleanupManager.trackResource(
        'context',
        `action_${actionId}`,
        async () => {
  ...
  private queueAction(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.config.debug) {
        console.log(
          `[ActionHandler] Action ${actionId} queued (queue size: ${this.processingQueue.length})`
        );
      }
    });
  }
   ...
   */
  private updateStatistics(success: boolean, processingTime: number): void {
    if (success) {
    ...
    this.statistics.averageProcessingTime =
      this.statistics.totalProcessingTime / this.statistics.totalActions;

    if (
      processingTime < this.statistics.performance.fastestAction ||
      this.statistics.performance.fastestAction === 0
    ) {
   ...
   */
  private updateConcurrentStats(): void {
   ...
   */
  private initializeStatistics(): ActionHandlerStatistics {
    return {
      totalActions: 0,
      successfulActions: 0,
      failedActions: 0,
