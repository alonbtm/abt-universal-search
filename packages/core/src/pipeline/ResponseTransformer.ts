...
import { AdvancedDataValidator, ValidationFunctions } from '../utils/DataValidator';
...
export function createEnhancementRule() {
  let ruleName = '';
  let rulePriority = 0;
  let conditionFn: (result: SearchResult) => boolean = () => true;
  let enhanceFn: (result: SearchResult, context: any) => Record<string, any> = () => ({});
  ...
  return {
    ...
    when(field: string, operator: 'equals' | 'contains', value: any) {
      conditionFn = (result: SearchResult) => {
...
export class ResponseTransformer {
  private readonly config: ResponseMapping;
  ...
  private transformSingleResult(
    ...
    if (!rawResult || !rawResult.item) return null;
   ...
   */
  private generateDescription(item: any, metadata: any): string | undefined {
   ...
   */
  private generateResultId(item: any, index: number): string | number {
...
export interface EnhancedFieldMappingWithComplex {
  ...
  validationRules?: Record<string, Array<{
    validate: (value: unknown, context?: any) => ValidationResult | boolean | { valid: boolean; message?: string };
...
export interface TransformationPipelineConfig {
  ...
  enableEnhancement?: boolean; // Alias for test compatibility
  /** Enable result filtering */
  enableResultFiltering?: boolean;
  enableFiltering?: boolean; // Alias for test compatibility
...
export class AdvancedResponseTransformer {
  ...
  private metadataEnhancer: AdvancedMetadataEnhancer;
  private resultFilter: AdvancedResultFilter;
  ...
  private filteredResultsCount = 0;
  ...
  public async transformResults(
    rawResults: RawSearchResult[],
    context: EnhancedTransformationContext
   ...
   */
  public async transformSingleResult(
    rawResult: RawSearchResult,
    context: EnhancedTransformationContext
   ...
   */
  public updateConfiguration(newConfig: Partial<TransformationPipelineConfig>): void {
   ...
   */
  public getStats(): typeof this.performanceStats & {
   ...
   */
  public clearCachesAndStats(): void {
    this.performanceStats = {
      totalTransformations: 0,
      totalProcessingTime: 0,
      averageProcessingTime: 0,
   ...
   */
  public getStatistics(): typeof this.performanceStats {
   ...
   */
  private async performBasicTransformation(
    rawResults: RawSearchResult[],
    context: EnhancedTransformationContext
  ...
  private transformSingleRawResult(
    ...
    const item = rawResult.item as Record<string, unknown>;
    if (!item || typeof item !== 'object') return null;
   ...
   */
  private async performMetadataEnhancement(
    results: SearchResult[],
    rawResults: RawSearchResult[],
    context: EnhancedTransformationContext
  ): Promise<SearchResult[]> {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      try {
        ...
        let enhancedResult: SearchResult;
   ...
   */
  private async performResultFiltering(
    results: SearchResult[],
    rawResults: RawSearchResult[],
    context: EnhancedTransformationContext
  ): Promise<SearchResult[]> {
   ...
   */
  private extractFieldValue(obj: Record<string, unknown>, field: string): unknown {
  ...
  private extractComplexFieldValue(
    ...
    fieldMapping: string | ComplexFieldMapping
  ): unknown {
   ...
   */
  private processTemplate(template: string, obj: Record<string, unknown>): string {
   ...
   * Generate description from item or metadata
   */
  private generateDescription(item: Record<string, unknown>, metadata: any): string | undefined {
    if (metadata.subtitle && typeof metadata.subtitle === 'string') {
      return metadata.subtitle;
    }
   ...
   * Extract URL from item
   */
  private extractUrl(item: Record<string, unknown>): string | undefined {
    const urlFields = ['url', 'link', 'href', 'website'];
    for (const field of urlFields) {
   ...
   * Generate unique ID for search result
   */
  private generateResultId(item: Record<string, unknown>, index: number): string | number {
    const idFields = ['id', '_id', 'uuid', 'key'];
    for (const field of idFields) {
  ...
  private initializePipeline(): void {
    ...
    if (this.config.enableValidation && this.config.validationRules) {
      ...
      for (const rule of this.config.validationRules) {
        const validationRule = {
          ...
          severity: rule.required ? 'error' : 'warning' as const,
    ...
    if (this.config.enableResultFiltering) {
      ...
      if (this.config.minQualityScore !== undefined) {
        this.resultFilter.addRule(
          CommonFilterRules.minimumQuality(this.config.minQualityScore)
        );
      }
    }
  }
   ...
   */
  private validateAndNormalizeConfig(config: TransformationPipelineConfig): TransformationPipelineConfig {
   ...
   */
  private getEnhancementCount(): number {
   ...
   */
  private collectErrors(): Array<{ message: string; index?: number; field?: string; }> {
    const errors = [];
   ...
   */
  private generateDefaultIcon(result: SearchResult): string {
