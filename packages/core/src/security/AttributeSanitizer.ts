 ...
 */
export const DEFAULT_ATTRIBUTE_SANITIZATION_CONFIG: AttributeSanitizationConfig = {
  allowedAttributes: [
...
export class AttributeSanitizer {
  private config: AttributeSanitizationConfig;
  ...
  public sanitizeAttributes(
    ...
    customConfig?: Partial<AttributeSanitizationConfig>
  ): AttributeSanitizationResult {
    const config = customConfig ? { ...this.config, ...customConfig } : this.config;
    const sanitizedAttributes: Record<string, string> = {};
    const removedAttributes: string[] = [];
    const modifiedValues: Record<string, { original: string; sanitized: string }> = {};
   ...
   */
  public sanitizeMetadata(metadata: Record<string, any>): AttributeSanitizationResult {
    const metadataConfig: AttributeSanitizationConfig = {
      ...this.config,
      allowedAttributes: [
   ...
   */
  public sanitizeStyleAttribute(styleValue: string): string {
   ...
   */
  public validateDataAttribute(attributeName: string, value: string): boolean {
    ...
    if (this.config.allowedDataPrefixes && this.config.allowedDataPrefixes.length > 0) {
      const isAllowed = this.config.allowedDataPrefixes.some(prefix => 
   ...
   * Check if attribute is allowed
   */
  private isAttributeAllowed(attributeName: string, config: AttributeSanitizationConfig): boolean {
  ...
  private sanitizeAttributeValue(
    ...
    value: string,
    config: AttributeSanitizationConfig
  ): string {
    let sanitized = value;
   ...
   */
  private sanitizeURLAttribute(value: string): string {
   ...
   */
  private sanitizeClassAttribute(value: string): string {
    return value
      .split(/\s+/)
   ...
   */
  private sanitizeIdAttribute(value: string): string {
   ...
   */
  private sanitizeGenericAttribute(value: string): string {
   ...
   */
  private isCSSPropertySafe(property: string, value: string): boolean {
   ...
   */
  private isValueSafe(attributeName: string, value: string): boolean {
   ...
   */
  public batchSanitize(
    attributeSets: Record<string, any>[],
    config?: Partial<AttributeSanitizationConfig>
  ): AttributeSanitizationResult[] {
   ...
   */
  public getSanitizationStats(attributes: Record<string, any>): {
   ...
   */
  public updateConfig(newConfig: Partial<AttributeSanitizationConfig>): void {
   ...
   */
  public getConfig(): AttributeSanitizationConfig {
    return { ...this.config };
  }
}
