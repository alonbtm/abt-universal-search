...
export class ErrorClassifier implements IErrorClassifier {
  private rules: Map<string, ErrorClassificationRule> = new Map();
  private metrics: ClassifierMetrics;
  private defaultRules: ErrorClassificationRule[] = [];
  ...
  constructor() {
    this.metrics = {
      ...
      averageConfidence: 0,
      classificationsByType: {} as Record<ErrorType, number>,
      classificationsBySeverity: {} as Record<ErrorSeverity, number>,
      rulePerformance: {},
    };
  ...
  private evaluateRule(
    ...
    if (
      matcher.codePattern &&
      (error as any).code &&
      !matcher.codePattern.test((error as any).code)
    ) {
      return false;
    }

    // Check status code
    if (matcher.statusCode && (error as any).statusCode !== matcher.statusCode) {
      return false;
    }
    ...
    if (
      (matcher as any).stackPattern &&
      error.stack &&
      !(matcher as any).stackPattern.test(error.stack)
    ) {
      return false;
    }
  ...
  private createDefaultClassification(error: Error, context?: ErrorContext): ErrorClassification {
    ...
    let recoverability: ErrorRecoverability = 'unknown';
  ...
  private applyRule(
    ...
    const recoverability = this.refineRecoverability(
      baseClassification.recoverability!,
      error,
      context
    );
  ...
  private refineRecoverability(
    baseRecoverability: ErrorRecoverability,
    error: Error,
    context?: ErrorContext
  ): ErrorRecoverability {
    // Configuration errors in production are less likely to be recoverable
    if (
      baseRecoverability === 'recoverable' &&
  ...
  private getDefaultClassification(error: Error): ErrorClassification {
    let type: ErrorType = 'unknown';
    let severity: ErrorSeverity = 'medium';
    let recoverability: ErrorRecoverability = 'unknown';
  ...
  private updateClassificationMetrics(classification: ErrorClassification, ruleId: string): void {
    // Update type metrics
    ...
    this.metrics.averageConfidence =
      (this.metrics.averageConfidence * (totalClassifications - 1) + classification.confidence) /
      totalClassifications;
  }

  public registerRule(rule: ErrorClassificationRule): void {
  ...
  public updateRuleWeights(performance: ClassificationPerformance): void {
  ...
  public getMetrics(): ClassifierMetrics {
    return { ...this.metrics };
  }
  ...
  public importRules(rulesJson: string): void {
    try {
      const rules: ErrorClassificationRule[] = JSON.parse(rulesJson);
