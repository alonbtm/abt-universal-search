...
export class FontManager {
  ...
  private loadingFonts = new Map<string, FontLoadingState>();
  private fontStacks = new Map<WritingSystem, FontStack>();
  private eventListeners: Map<keyof InternationalizationEvents, Function[]> = new Map();
  
  // Font face observer for detection
  private fontFaceObserver: any = null; // FontFaceObserver if available
  
  // Font loading timeout
  private readonly FONT_LOAD_TIMEOUT = 10000; // 10 seconds

  // Default font stacks by writing system
  private readonly defaultFontStacks: Record<WritingSystem, FontStack> = {
    latin: {
  ...
  private readonly webFontUrls: Record<string, string> = {
   ...
   */
  public getFontStackForText(text: string): string[] {
   ...
   */
  public getFontStackForWritingSystem(writingSystem: WritingSystem): string[] {
   ...
   */
  public getFontStackForLocale(locale: LocaleCode): string[] {
    const language = locale.split('-')[0];
    
    // Map languages to writing systems
    const languageToScript: Record<string, WritingSystem> = {
      'ar': 'arabic',
      'he': 'hebrew',
      'zh': 'cjk',
      'ja': 'cjk',
  ...
  public async loadFont(
    ...
    const loadingState: FontLoadingState = {
      family,
      status: 'loading',
      startTime: Date.now()
    };
    ...
    try {
      ...
      this.emit('font-error', family, error as Error);
   ...
   */
  public isFontLoading(family: string): boolean {
   ...
   */
  public getFontLoadingStatus(family: string): FontLoadingState | null {
   ...
   */
  public applyFontStack(element: HTMLElement, text?: string, writingSystem?: WritingSystem): void {
   ...
   */
  public getSupportedScripts(family: string): WritingSystem[] {
    const scriptMap: Record<string, WritingSystem[]> = {
      'Inter': ['latin'],
      'Noto Sans': ['latin'],
  ...
  private initializeFontStacks(): void {
    // Merge default font stacks with custom configuration
    Object.entries(this.defaultFontStacks).forEach(([script, stack]) => {
      const customStack = this.config.fallbackFonts[script as WritingSystem];
      
      if (customStack) {
        this.fontStacks.set(script as WritingSystem, {
          primary: [...(Array.isArray(customStack) ? customStack : []), ...stack.primary],
          fallback: stack.fallback,
          system: stack.system
        });
      } else {
        this.fontStacks.set(script as WritingSystem, stack);
      }
    });
  }

  private checkFontFaceSupport(): void {
  ...
  private async preloadFonts(fonts: string[]): Promise<void> {
  ...
  private async loadFontViaCSS(family: string, url: string): Promise<void> {
  ...
  private injectOptimizationCSS(): void {
    const style = document.createElement('style');
  ...
  private preconnectFontCDNs(): void {
    const cdns = [
  ...
  private removeUnusedFontVariations(): void {
  ...
  private getDefaultFontMetrics(family: string, size: number): FontMetrics {
    return {
      family,
      size,
      lineHeight: size * 1.2,
