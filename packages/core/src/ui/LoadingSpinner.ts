...
import type { AccessibilityConfig, ScreenReaderAnnouncement } from '../types/Accessibility';
...
export class LoadingSpinner {
  ...
  private readonly config: LoadingSpinnerConfig;
  ...
  private eventListeners: Map<keyof LoadingSpinnerEvents, Function[]> = new Map();
   ...
   */
  public async start(message: string = 'Loading...'): Promise<void> {
    if (!this.isInitialized) {
      await this.init();
    }
   ...
   */
  public setTimeout(timeoutMs: number, message?: string): void {
   ...
   * Add event listener
   */
  public on<K extends keyof LoadingSpinnerEvents>(
    event: K,
    handler: LoadingSpinnerEvents[K]
  ): void {
   ...
   */
  public off<K extends keyof LoadingSpinnerEvents>(
    event: K,
    handler: LoadingSpinnerEvents[K]
  ): void {
  ...
  private applyAccessibilityAttributes(): void {
    if (!this.spinnerElement || !this.accessibilityManager) return;
   ...
   */
  private announceToScreenReader(announcement: ScreenReaderAnnouncement): void {
    if (this.screenReaderManager) {
      this.screenReaderManager.announce(announcement);
    }
  }

  private createSpinnerStructure(): void {
  ...
  private clearProgressUpdate(): void {
  ...
  private emit<K extends keyof LoadingSpinnerEvents>(
    event: K,
    ...args: Parameters<LoadingSpinnerEvents[K]>
  ): void {
    const listeners = this.eventListeners.get(event);
    if (listeners) {
  ...
  private initializeEventMaps(): void {
    const events: (keyof LoadingSpinnerEvents)[] = ['timeout', 'start', 'stop'];
   ...
   * Get localized text
   */
  private getText(key: string, context?: any): string {
