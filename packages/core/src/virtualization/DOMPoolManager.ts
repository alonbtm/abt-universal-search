...
export class DOMPoolManager implements IDOMPoolManager {
  private config: Required<DOMPoolConfig>;
  private cullingConfig: ViewportCullingConfig;
  private events: DOMPoolEvents;
  private factory: ElementFactory;
  
  private pools: Map<string, PooledElement[]> = new Map();
  private inUseElements: Set<HTMLElement> = new Set();
  private elementMetadata: Map<HTMLElement, ElementMetadata> = new Map();
  
  private cleanupTimer: NodeJS.Timeout | null = null;
  private memoryMonitorTimer: NodeJS.Timeout | null = null;
  
  private stats: DOMPoolStats = {
    totalElements: 0,
    availableElements: 0,
    inUseElements: 0,
    reuseRate: 0,
  ...
  constructor(
    config: Partial<DOMPoolConfig> = {},
    cullingConfig: Partial<ViewportCullingConfig> = {},
    events: DOMPoolEvents = {},
  ...
  private createDefaultFactory(): ElementFactory {
    return {
  ...
  private createInitialPool(tagName: string): void {
  ...
  private getPoolKey(tagName: string, className?: string): string {
    return className ? `${tagName}.${className}` : tagName;
  }

  public acquireElement(tagName: string, className?: string): HTMLElement {
  ...
  public releaseElement(element: HTMLElement): void {
    if (!this.config.enablePooling) {
    ...
    let pooledElement: PooledElement | undefined;
    let pool: PooledElement[] | undefined;
    
    for (const [, currentPool] of Array.from(this.pools)) {
      pooledElement = currentPool.find((pe: any) => pe.element === element);
      if (pooledElement) {
        pool = currentPool;
        break;
      }
  ...
  private setElementMetadata(element: HTMLElement, metadata: ElementMetadata): void {
    this.elementMetadata.set(element, metadata);
  }

  public getPoolStats(): DOMPoolStats {
    return { ...this.stats };
  }
  ...
  private updateStats(): void {
    ...
    for (const pool of Array.from(this.pools.values())) {
      ...
      availableElements += pool.filter((pe: any) => !pe.isInUse).length;
      inUseElements += pool.filter((pe: any) => pe.isInUse).length;
    }
  ...
  public setMaxPoolSize(size: number): void {
    ...
    for (const pool of Array.from(this.pools.values())) {
      while (pool.length > size) {
        const pooledElement = pool.find((pe: any) => !pe.isInUse);
        if (pooledElement) {
          const index = pool.indexOf(pooledElement);
          pool.splice(index, 1);
          this.destroyElement(pooledElement.element);
  ...
  public cullInvisibleElements(viewport: { top: number; bottom: number }): void {
    if (!this.cullingConfig.enableCulling) return;

    const culledElements: HTMLElement[] = [];
  ...
  private shouldCullElement(element: HTMLElement, viewport: { top: number; bottom: number }): boolean {
    if (!element.parentNode) return true; // Already detached
  ...
  public getElementMetadata(element: HTMLElement): ElementMetadata | null {
    return this.elementMetadata.get(element) || null;
  }

  public preWarmPool(tagName: string, className?: string, count: number = 10): void {
    const poolKey = this.getPoolKey(tagName, className);
    let pool = this.pools.get(poolKey);
