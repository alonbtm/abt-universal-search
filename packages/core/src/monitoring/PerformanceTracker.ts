...
export class PerformanceTracker {
  private measurements: Map<string, SearchPerformanceMeasurement[]>;
  ...
  private measurementIdCounter: number = 0;
  ...
  startMeasurement(operation: string, metadata?: Record<string, any>): string {
    ...
    const measurement: SearchPerformanceMeasurement = {
      id,
      operation,
    ...
    if (typeof performance !== 'undefined' && (performance as any).memory) {
      measurement.memoryBefore = (performance as any).memory.usedJSHeapSize;
    }
  ...
  endMeasurement(
    ...
    let measurement: SearchPerformanceMeasurement | undefined;
    let operationType: string = '';

    for (const [operation, measurements] of Array.from(this.measurements.entries())) {
      const found = measurements.find((m: any) => m.id === measurementId);
      if (found) {
        measurement = found;
    ...
    if (typeof performance !== 'undefined' && (performance as any).memory) {
      measurement.memoryAfter = (performance as any).memory.usedJSHeapSize;
    }
  ...
  establishBaseline(
    name: string,
    targetResponseTime: number,
    targetRenderTime: number = 16.67, // 60fps
    targetInteractionLatency: number = 100,
    regressionThreshold: number = 0.2 // 20% degradation
  ): void {
  ...
  detectRegressions(operation: string): RegressionDetection {
    ...
    else if (maxDegradation > 0.3) severity = 'moderate';
  ...
  getMetricsSummary(operation?: string, timeWindow?: number): {
    ...
    let allMeasurements: SearchPerformanceMeasurement[] = [];
    ...
    if (operation) {
      ...
      for (const measurements of Array.from(this.measurements.values())) {
        allMeasurements.push(...measurements.filter((m: any) => m.startTime >= cutoff));
      }
    }
    ...
    for (const measurement of interactionMeasurements) {
      ...
      if (!interactionsByType[type]) interactionsByType[type] = [];
      ...
      this.interactionObserver = new PerformanceObserver((list) => {
        for (const entry of list.getEntries()) {
          if (entry.entryType === 'event') {
            ...
            let interactionType: 'click' | 'keyboard' | 'touch' | 'voice' = 'click';
            if (eventEntry.name.includes('key')) interactionType = 'keyboard';
            else if (eventEntry.name.includes('touch')) interactionType = 'touch';
  ...
  private establishDefaultBaselines(): void {
  ...
  private checkForRegressions(operation: string, measurement: SearchPerformanceMeasurement): void {
    // Only check every 10th measurement to avoid performance impact
    if (this.measurementIdCounter % 10 !== 0) return;
  ...
  private generateRegressionRecommendations(metrics: string[], severity: 'minor' | 'moderate' | 'severe'): string[] {
...
// Type definitions for Performance Event Timing API
