...
import type { Connection, SearchError } from '../types/Results';
...
export class ConnectionPool {
   ...
   */
  public async acquire(config: DataSourceConfig): Promise<Connection> {
    if (this.isDestroyed) {
      throw errorMapper.mapError(
        new Error('Connection pool has been destroyed'),
        this.adapterType,
        { config }
      );
    }
  ...
  public async destroy(): Promise<void> {
    ...
    const waitingError = errorMapper.mapError(
      new Error('Connection pool destroyed'),
      this.adapterType
    );
   ...
   */
  private async findIdleConnection(): Promise<Connection | null> {
   ...
   */
  private async createNewConnection(config: DataSourceConfig): Promise<Connection> {
    try {
      const connection = await Promise.race([
        this.factory.createConnection(config),
        this.createTimeoutPromise<Connection>(this.config.connectionTimeoutMs)
      ]);
   ...
   */
  private async waitForConnection(config: DataSourceConfig): Promise<Connection> {
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(errorMapper.mapError(
          new Error('Connection acquire timeout'),
          this.adapterType,
          { config, timeout: this.config.acquireTimeoutMs }
        ));
   ...
   */
  private prepareConnection(connection: Connection, config: DataSourceConfig): Connection {
    const pooledConnection = this.pool.get(connection.id)!;
    pooledConnection.inUse = true;
    pooledConnection.lastUsed = Date.now();
