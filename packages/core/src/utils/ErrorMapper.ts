...
export class ErrorMapper {
  private customMappings = new Map<string, ErrorMappingConfig>();
  ...
  public mapError(
    error: unknown,
    adapterType: string,
    context?: Partial<SearchError['context']>
  ): SearchError {
    const baseError = this.extractBaseError(error);
    const mapping = this.findMapping(baseError);
    
    const searchError = new Error(baseError.message) as SearchError;
   ...
   */
  public getRecoverySuggestions(error: unknown): string[] {
    const baseError = this.extractBaseError(error);
    const mapping = this.findMapping(baseError);
   ...
   */
  private extractBaseError(error: unknown): { message: string; code?: string; statusCode?: number } {
   ...
   */
  private findMapping(baseError: { message: string; code?: string; statusCode?: number }): ErrorMappingConfig {
   ...
   */
  public getRegisteredErrorTypes(): string[] {
    return [
   ...
   */
  public clearCustomMappings(): void {
