 ...
 */
export function detectTextDirection(text: string, threshold: number = 0.3): DirectionDetectionResult {
  let ltrChars = 0;
  let rtlChars = 0;
  let neutralChars = 0;
  ...
  const direction: TextDirection = rtlRatio >= threshold ? 'rtl' : 'ltr';
 ...
 */
export function isRTLCharacter(codePoint: number): boolean {
 ...
 */
export function isLTRCharacter(codePoint: number): boolean {
 ...
 */
export function detectWritingSystem(text: string): WritingSystem[] {
  const systems = new Set<WritingSystem>();
  ...
  for (const char of text) {
    ...
    for (const [system, ranges] of Object.entries(UNICODE_RANGES)) {
      if (ranges.some(([start, end]) => codePoint >= start && codePoint <= end)) {
        systems.add(system as WritingSystem);
      }
    }
  }
 ...
 */
export function getLanguageFromLocale(locale: LocaleCode): string {
 ...
 */
export function getRegionFromLocale(locale: LocaleCode): string {
 ...
 */
export function isLocaleSupported(locale: LocaleCode): boolean {
  try {
...
export function getBestMatchingLocale(
  ...
  const languageMatch = supportedLocales.find(
    locale => getLanguageFromLocale(locale) === requestedLanguage
  );
 ...
 */
export function analyzeUnicodeText(text: string): UnicodeAnalysisResult {
 ...
 */
export function analyzeBidirectionalText(text: string): BidiAnalysisResult {
  const direction = detectTextDirection(text);
  const baseDirection = direction.direction;
  const segments: BidiSegment[] = [];
  
  let currentSegment = '';
  let currentDirection: TextDirection = baseDirection;
  let startIndex = 0;
 ...
 */
export function calculateRTLPosition(
 ...
 */
export function toLogicalProperties(
 ...
 */
export function supportsLogicalProperties(): boolean {
